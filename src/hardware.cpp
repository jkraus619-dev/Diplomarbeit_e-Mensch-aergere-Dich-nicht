...
uint32_t colors[] = {
    ...
    strip.Color(0, 0, 200),    // Blau
    strip.Color(0, 180, 0),    // Grün
    strip.Color(200, 200, 0),  // Gelb
    strip.Color(200, 200, 200) // weiß
};
...
void buttonInputLoop()
{
    ...
    // Nur im Offline/Button-Modus aktiv
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        // Fallende Flanke erkannt (HIGH -> LOW = Tastendruck)
        ...
        if (state == LOW && lastState[i] == HIGH)
        {
            ...
            if (now - lastPressTime[i] >= DEBOUNCE_MS)
            {
                ...
                handleButtonPress(i); // 0=Select, 1=Next, 2=Cancel
            }
        }

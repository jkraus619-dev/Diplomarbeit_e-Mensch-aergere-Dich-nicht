...
void sdcard_logBatteryVoltage(float voltage) {
  ...
  if (now < 1700000000) return;    // Uhrzeit noch nicht synchronisiert
  if (voltage < 3.0f) return;      // Messfehler oder kein Akku

  File f = SD.open(CSV_PATH, FILE_APPEND);
  if (!f) return;
  f.printf("%lu,%.2f\n", (unsigned long)now, voltage);
  f.close();

  // Gleitendes Fenster: mehr als 200 Eintraege -> aeltesten loeschen

  // Pass 1: Datenzeilen zaehlen
  ...
  while (f.available()) {
    ...
    line.trim();
    if (!line.length()) continue;
    if (firstLine) { firstLine = false; continue; } // Header ueberspringen
    count++;
  }
  f.close();

  if (count <= 100) return; // nichts zu tun

  // Pass 2: nur die letzten 100 Eintraege behalten
  static const char* TMP_PATH = "/battery_tmp.csv";
  f = SD.open(CSV_PATH, FILE_READ);
  if (!f) return;
  File tmp = SD.open(TMP_PATH, FILE_WRITE);
  if (!tmp) { f.close(); return; }

  int skip = count - 100; // so viele alte Eintraege weglassen
  firstLine = true;
  int skipped = 0;
  while (f.available()) {
    String line = f.readStringUntil('\n');
    ...
    if (skipped < skip) { skipped++; continue; } // aelteste weglassen
    tmp.print(line + "\n");
  }
  f.close();
  tmp.close();
...
String sdcard_getBatteryJson() {
  ...
  while (f.available()) {
    ...
    if (tsStr.toInt() < 1700000000) continue;  // Boot-Timestamp
    if (vStr.toFloat() < 3.0f) continue;       // Messfehler
    if (!first) json += ",";
    json += "{\"t\":" + tsStr + ",\"v\":" + vStr + "}";
    first = false;
  }
